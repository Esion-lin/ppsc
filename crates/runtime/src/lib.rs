...
pub mod manifest;
pub mod manifest_crypto_process;
pub mod manifest_daemon;
...
impl MpcAmount {
    ...
    /// Construct from backend-serialized bytes (used by real MPC backends to hand a share
    /// batch back to the runtime as an opaque handle).
    pub fn from_backend_bytes(bytes: Vec<u8>) -> Self {
        Self::new(bytes)
    }

    /// Expose the backend-serialized bytes to a real MPC backend. Callers must not log or
    /// output these bytes.
    pub fn backend_bytes(&self) -> &[u8] {
        self.expose_to_backend()
    }
}
