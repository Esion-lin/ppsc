...
fhe_ciphertext fhe_eval_sub(fhe_ctx ctx, fhe_ciphertext a, fhe_ciphertext b) {
    try {
        auto cc = *static_cast<CtxType*>(ctx);
        auto& ar = *static_cast<CtType*>(a);
        auto& br = *static_cast<CtType*>(b);
        return new CtType(cc->EvalSub(ar, br));
    } catch (...) {
        return nullptr;
    }
}
...
fhe_ctx fhe_bfv_new(uint32_t mult_depth, uint32_t batch_size) {
    try {
        ...
        params.SetPlaintextModulus(998244353ULL);
        params.SetBatchSize(batch_size);
        params.SetMultiplicativeDepth(mult_depth);
...
// Client-side encryption needs only the serialized aggregate public key. OpenFHE
// serializes its crypto context together with the key; no secret key is imported.
extern "C" uint8_t* fhe_bfv_encrypt_public(const uint8_t* data, size_t size,
                                         int64_t amount, size_t* len) {
    try {
        PublicKey<DCRTPoly> pk;
        if (!deserialize_from_bytes(pk, data, size) || !pk) return nullptr;
        auto cc = pk->GetCryptoContext();
        auto plaintext = cc->MakePackedPlaintext(std::vector<int64_t>{amount});
        return serialize_to_bytes(cc->Encrypt(pk, plaintext), len);
    } catch (...) {
        return nullptr;
    }
}

extern "C" bool fhe_validate_public_ciphertext(const uint8_t* key, size_t key_len,
                                                const uint8_t* data, size_t data_len) {
    try {
        PublicKey<DCRTPoly> pk;
        CtType ct;
        return deserialize_from_bytes(pk, key, key_len) && pk &&
               deserialize_from_bytes(ct, data, data_len) && ct &&
               ct->GetKeyTag() == pk->GetKeyTag() &&
               ct->GetCryptoContext() == pk->GetCryptoContext() &&
               ct->GetElements().size() == 2;
    } catch (...) { return false; }
}
