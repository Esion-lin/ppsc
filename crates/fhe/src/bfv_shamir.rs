...
//!
//! BFV's plaintext space is a single modulus `p = 998244353`, used only to **store** encrypted
...
fn int_to_field(x: i64) -> Fr {
    Fr::from(x.rem_euclid(998244353) as u64)
}
...
mod tests {
    ...
    fn bfv_share_plaintext_threshold_works() {
        ...
        for (i, share_vec) in shares.iter().enumerate() {
            ...
            assert_eq!(field_to_int(v), data[i] % 998244353, "slot {i}");
        }
    }
    ...
    fn bfv_scale_partial_control() {
        ...
        for (i, (a, b)) in dec.iter().zip(data.iter()).enumerate() {
            assert_eq!(*a, (2 * b) % 998244353, "scaled slot {i}");
        }
    }
    ...
    fn bfv_add_partials_control() {
        ...
        let dec = ctx.bfv_fusion(&[comb], 8).expect("fusion");
        for (i, (a, b)) in dec.iter().zip(data.iter()).enumerate() {
            assert_eq!(*a, (2 * b) % 998244353, "added slot {i}");
        }
    }
