...
extern "C" {
    fn fhe_bfv_encrypt_public(data: *const u8, size: usize, amount: i64, len: *mut usize) -> *mut u8;
    fn fhe_validate_public_ciphertext(key: *const u8, key_len: usize, data: *const u8, data_len: usize) -> bool;
    fn fhe_ckks_new(
        mult_depth: u32,
        scale_mod_size: u32,
        batch_size: u32,
    ...
    fn fhe_eval_sub(ctx: *mut c_void, a: *mut c_void, b: *mut c_void) -> *mut c_void;
...
/// Largest nonnegative integer represented without signed BFV plaintext wrap.
pub const BFV_MAX_AMOUNT: u128 = (998_244_353 - 1) / 2;

/// Check serialization, key tag and context without opening the ciphertext.
pub fn validate_public_ciphertext(key: &[u8], ciphertext: &[u8]) -> bool {
    unsafe { fhe_validate_public_ciphertext(key.as_ptr(), key.len(), ciphertext.as_ptr(), ciphertext.len()) }
}

/// Encrypt with a committee's serialized public key, without any private keys.
pub fn encrypt_public_amount(public_key: &[u8], amount: u128) -> Option<Vec<u8>> {
    if amount > BFV_MAX_AMOUNT || public_key.is_empty() {
        return None;
    }
    let mut len = 0;
    let ptr = unsafe {
        fhe_bfv_encrypt_public(public_key.as_ptr(), public_key.len(), amount as i64, &mut len)
    };
    if ptr.is_null() { None } else { Some(take_bytes(ptr, len)) }
}
...
// The OpenFHE CryptoContext / KeyPair / Ciphertext are not thread-safe C++ objects; the
// real backend uses them single-threaded. Marking Send/Sync lets that backend satisfy the
// runtime's `Send + Sync` trait bound under the single-threaded usage assumption.
unsafe impl Send for CkksContext {}
unsafe impl Sync for CkksContext {}
...
impl CkksContext {
    ...
    pub fn eval_sub(&self, a: &Ciphertext, b: &Ciphertext) -> Option<Ciphertext> {
        let raw = unsafe { fhe_eval_sub(self.raw, a.raw, b.raw) };
        if raw.is_null() {
            None
        } else {
            Some(Ciphertext { raw })
    ...
    /// BFV context (integer plaintext, single plaintext modulus p = 998244353).
