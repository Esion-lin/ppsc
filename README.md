...
PPSC 是一个密态智能合约研究原型。当前的主流程不是“编译完在本地看 JSON”，而是一条已经接通的链：
...
请按 [编译、部署、调用、回写完整演示](docs/README-compiled-contract-e2e.md) 逐步运行。它不使用测试函数，也不使用一键 Shell 脚本；每一步都给出命令和预期输出。
