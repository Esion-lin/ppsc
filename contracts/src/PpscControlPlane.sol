...
contract PpscControlPlane {
    ...
    mapping(bytes32 => bytes4) private _executionSelectors;
    mapping(bytes32 => bytes32[]) private _executionInputs;
    mapping(bytes32 => bytes) private _executionPublicInputs;
    ...
    /// @notice Generic compiler entrypoint. Public ABI values are encoded in
    /// `publicInputs`; protected parameters remain dataId references.
    function invokeCompiled(
        bytes32 contractId,
        bytes4 selector,
        bytes32[] calldata inputIds,
        ...
        return _invoke(contractId, selector, inputIds, publicInputs, msg.sender, nonce, deadline);
    }

    /// @notice Generated application gateways use this entrypoint so the
    /// execution requester remains the end user rather than the gateway.
    function invokeCompiledFor(
        bytes32 contractId,
        bytes4 selector,
        bytes32[] calldata inputIds,
        bytes calldata publicInputs,
        ...
        return _invoke(contractId, selector, inputIds, publicInputs, requester, nonce, deadline);
    }
    ...
    function _invoke(
        ...
        bytes32 inputRoot = keccak256(abi.encode(inputIds, keccak256(publicInputs)));
        ...
        _executionSelectors[executionId] = selector;
        _executionPublicInputs[executionId] = publicInputs;
        for (uint256 i; i < inputIds.length; ++i) {
            _executionInputs[executionId].push(inputIds[i]);
        }
        _executionIds.push(executionId);
        emit ExecutionRequested(executionId, contractId, selector, requester, inputRoot);
    }

    /// @notice Durable task queue used by daemons after restart. The input root
    /// remains the commitment checked when the request was created.
    function executionCount() external view returns (uint256) {
        return _executionIds.length;
    }

    function executionIdAt(uint256 index) external view returns (bytes32) {
        return _executionIds[index];
    }

    function executionRequest(bytes32 executionId)
        external
        view
        returns (
            bytes32 contractId,
            bytes4 selector,
            bytes32[] memory inputIds,
            bytes memory publicInputs,
            address requester,
            uint64 deadline,
            ExecutionStatus status
        )
    {
        ...
        if (execution.status == ExecutionStatus.None) revert NotFound();
        return (
            execution.contractId,
            _executionSelectors[executionId],
            _executionInputs[executionId],
            _executionPublicInputs[executionId],
            execution.requester,
            execution.deadline,
            execution.status
        );
    ...
    function executionSelector(bytes32 executionId) external view returns (bytes4) {
        if (executions[executionId].status == ExecutionStatus.None) revert NotFound();
        return _executionSelectors[executionId];
    }

    function executionInputCount(bytes32 executionId) external view returns (uint256) {
        if (executions[executionId].status == ExecutionStatus.None) revert NotFound();
        return _executionInputs[executionId].length;
    }

    function executionInputAt(bytes32 executionId, uint256 index) external view returns (bytes32) {
        if (executions[executionId].status == ExecutionStatus.None) revert NotFound();
        return _executionInputs[executionId][index];
    }

    function executionPublicInputs(bytes32 executionId) external view returns (bytes memory) {
        if (executions[executionId].status == ExecutionStatus.None) revert NotFound();
        return _executionPublicInputs[executionId];
    }

    function executionRequester(bytes32 executionId) external view returns (address) {
        Execution storage execution = executions[executionId];
        if (execution.status == ExecutionStatus.None) revert NotFound();
        return execution.requester;
    }
    ...
    function contractManifestHash(bytes32 contractId) external view returns (bytes32) {
        ConfidentialContract storage target = contracts[contractId];
        if (target.owner == address(0)) revert NotFound();
        return target.manifestHash;
    }
    ...
    function executionOldStateRoot(bytes32 executionId) external view returns (bytes32) {
        return executions[executionId].oldStateRoot;
    }

    function stateVariableExists(bytes32 variableAddress) external view returns (bool) {
        return stateVariables[variableAddress].exists;
    }
