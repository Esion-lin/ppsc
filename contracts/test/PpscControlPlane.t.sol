...
contract PpscControlPlaneTest {
    ...
    function testExecutionRequestCanBeRecoveredAfterDaemonRestart() public {
        bytes32[] memory inputs = new bytes32[](1);
        inputs[0] = inputDataId;
        bytes memory publicInputs = abi.encode(address(0xBEEF), uint256(7));
        bytes32 executionId = control.invokeCompiled(
            contractId, SELECTOR, inputs, publicInputs, 101, uint64(block.timestamp + 1 days)
        );

        require(control.executionCount() == 1, "execution not indexed");
        require(control.executionIdAt(0) == executionId, "wrong indexed execution");
        (
            bytes32 recoveredContract,
            bytes4 recoveredSelector,
            bytes32[] memory recoveredInputs,
            bytes memory recoveredPublicInputs,
            address recoveredRequester,,
            PpscControlPlane.ExecutionStatus recoveredStatus
        ) = control.executionRequest(executionId);
        require(recoveredContract == contractId, "wrong contract");
        require(recoveredSelector == SELECTOR, "wrong selector");
        require(recoveredInputs.length == 1 && recoveredInputs[0] == inputDataId, "wrong inputs");
        require(keccak256(recoveredPublicInputs) == keccak256(publicInputs), "wrong public inputs");
        require(recoveredRequester == address(this), "wrong requester");
        require(recoveredStatus == PpscControlPlane.ExecutionStatus.Requested, "wrong status");
        require(control.executionSelector(executionId) == SELECTOR, "selector getter failed");
        require(control.executionInputCount(executionId) == 1, "input count getter failed");
        require(control.executionInputAt(executionId, 0) == inputDataId, "input getter failed");
        require(
            keccak256(control.executionPublicInputs(executionId)) == keccak256(publicInputs),
            "public input getter failed"
        );
        require(control.executionRequester(executionId) == address(this), "requester getter failed");
        require(control.executionContract(executionId) == contractId, "contract getter failed");
    }

    function testCompiledGatewayPreservesEndUser() public {
        CompiledGatewayHarness gateway = new CompiledGatewayHarness(control);
        bytes32 gatewayContract = gateway.publish();
        address endUser = vm.addr(0x5151);
        bytes32 executionId = gateway.invokeAs(
            gatewayContract,
            endUser,
        ...
        require(control.executionRequester(executionId) == endUser, "end user not preserved");
    }
...
contract CompiledGatewayHarness {
    PpscControlPlane private immutable control;
    bytes4 private constant HARNESS_SELECTOR = bytes4(keccak256("privateTransfer(bytes32)"));

    constructor(PpscControlPlane control_) {
        control = control_;
    }

    function publish() external returns (bytes32 id) {
        id = control.publishContract(
            keccak256("gateway-harness"),
            keccak256("gateway-manifest"),
            keccak256("gateway-runtime"),
            keccak256("gateway-state")
        );
        control.publishFunction(
            id,
            HARNESS_SELECTOR,
            keccak256("gateway-program"),
            keccak256("gateway-operators"),
            keccak256("gateway-abi"),
            "file://gateway",
            100_000
        );
    }
    ...
    function invokeAs(
        ...
        return control.invokeCompiledFor(
            id, HARNESS_SELECTOR, privateInputs, publicInputs, requester, nonce, deadline
        );
    }
}
